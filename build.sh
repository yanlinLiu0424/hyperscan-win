...
download https://ftp.exim.org/pub/pcre/pcre-${PRCE_VER}.tar.bz2
...
if [ ! -d pcre-${PRCE_VER} ]; then tar jxf ../pcre-${PRCE_VER}.tar.bz2; fi
...
cmake-3.13.2-win64-x64/bin/cmake .. -G "$PLATFORM" -DBUILD_SHARED_LIBS=on  -DBOOST_ROOT=`cygpath -w $PWD`/boost_${BOOST_VER}/..
